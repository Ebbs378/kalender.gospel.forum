-- Gospel Forum Kalender V7 – Supabase Setup / Migration
...
-- Alte starre Kategorie-Constraint entfernen, damit Admins neue Kategorien anlegen können.
...
-- Admin-Zuordnung. Nutzer werden zuerst unter Authentication → Users erstellt.
...
as $$
  select exists(
    select 1 from public.calendar_admins a
...
-- Admins dürfen die eigene Admin-Zuordnung prüfen.
drop policy if exists "Admins can read own admin row" on public.calendar_admins;
create policy "Admins can read own admin row"
on public.calendar_admins for select
to authenticated
using (user_id=auth.uid());

-- Tabellen-/Spaltenrechte
...
-- 2) Danach diese Zeile mit der Admin-E-Mail ausführen:
--
-- insert into public.calendar_admins(user_id)
-- select id from auth.users where lower(email)=lower('DEINE-ADMIN-EMAIL@gospel-forum.de')
